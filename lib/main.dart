...
class RequestService {
  static Future<WikiSearchResponse?> query(String search) async {
    var response = await get(Uri.parse("https://en.wikipedia.org/w/api.php?action=query&origin=*&list=search&srsearch=$search&format=json"));
...
class WikiSearchEntity {
  ...
  String snippet;
  String timestamp;
  WikiSearchEntity({required this.ns, required this.title, required this.pageId, required this.size, required this.wordCount, required this.snippet, required this.timestamp});
